...
const Sidebar = ({ isOpen, onClose, user, currentPath }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full bg-white shadow-xl z-50 transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static w-64 flex flex-col`}>
        {/* Logo Section */}
        ...
        <div className="p-4 border-b border-gray-200" style={{ backgroundColor: COLORS.primary }}>
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-semibold truncate" style={{ color: COLORS.text }}>{user?.name}</p>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all ${currentPath === item.path ? 'text-white shadow-md' : 'text-gray-700 hover:bg-gray-100'}`}
        ...
        <div className="p-4 border-t border-gray-200">
          <p className="text-xs text-center text-gray-500">© 2026 ProductHub</p>
        </div>
      </aside>
    </>
  );
